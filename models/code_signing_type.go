...
func init() {
	...
	if err := json.Unmarshal([]byte(`["JAVASOFT","MS_AUTHENTICODE","VBA","ADOBE_AIR","APPLE"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
func (m CodeSigningType) validateCodeSigningTypeEnum(path, location string, value CodeSigningType) error {
	if err := validate.Enum(path, location, value, codeSigningTypeEnum); err != nil {
		return err
	}
	return nil
}
